    ...
    try {
      ...
      if (activeRes.success && activeRes.data) {
        ...
        if (activeRes.data.config) {
          setPolicyConfig(prev => ({ ...prev, ...activeRes.data.config }));
        }
      }
    ...
    <div className="max-w-6xl mx-auto pb-16 animate-in fade-in duration-500 space-y-8">
      ...
      <div className="bg-[#111] rounded-xl border border-[#333] overflow-hidden shadow-lg">
        <div className="px-6 py-4 border-b border-[#333] bg-[#161616] flex items-center justify-between">
          <h3 className="font-semibold text-white text-sm">Dynamic Policy Parameters</h3>
          <span className="text-xs text-gray-400">Pricing, eligibility, and escalation logic</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6">
          {[
            ['baseAnnualRatePercent', 'Base EMI Rate %', 0.25],
            ['maxFoirPercent', 'Eligibility FOIR Cap %', 1],
            ['highLoanEscalationAmount', 'High Loan L2 Limit', 50000],
            ['l1EscalationCount', 'L1 Deviations to L2', 1],
            ['eligibilityTenureMonths', 'Eligibility Tenure Months', 12],
            ['assetBufferThreshold', 'Asset Buffer Threshold', 25000],
          ].map(([field, label, step]) => (
            <label key={field} className="bg-[#181818] border border-[#2a2a2a] p-4 rounded-xl space-y-2">
              <span className="block text-xs text-gray-400 font-semibold">{label}</span>
              <input
                type="number"
                step={step}
                value={policyConfig[field] ?? ''}
                onChange={(e) => handleConfigChange(field, e.target.value)}
                className="w-full bg-[#111] border border-[#333] text-white px-3 py-2 rounded-lg text-sm font-bold font-mono focus:outline-none"
              />
            </label>
          ))}
        </div>
              ...
              <div className="w-full md:w-80 bg-[#181818] border border-[#2a2a2a] p-4 rounded-xl space-y-3">
                ...
                <div className="flex justify-between items-center text-xs pt-1">
                  ...
                  >
                    ...
                    <option value="EXCEPTION_L1">EXCEPTION (L1)</option>
                    <option value="EXCEPTION_L2">EXCEPTION (L2)</option>
                  </select>
                </div>
              </div>
            </div>
